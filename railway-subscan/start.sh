...
if [[ -n "${SLACK_BOT_TOKEN:-}" && -n "${SLACK_SUB_SCAN_CHANNEL_ID:-}" ]]; then
  if [[ -s /app/sub_scan_api_keys.csv ]]; then
    JSON_PAYLOAD=$(python - <<'PY'
import os, json
ch = os.environ["SLACK_SUB_SCAN_CHANNEL_ID"]
try:
    with open("/app/sub_scan_api_keys.csv","r") as f:
