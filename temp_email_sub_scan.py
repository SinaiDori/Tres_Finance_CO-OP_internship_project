...
# def wait_for_email_with_link(token, timeout=300, interval=5):
#     elapsed = 0
#     while elapsed < timeout:
#         try:
#             messages = get_messages(token)
#             print(f"📨 Found {len(messages)} messages in inbox")  # Debug line
...
#                 message = read_message(token, messages[0]["id"])
#                 print("📨 Raw email content:", message.get(
#                     "text") or message.get("html"))
#                 link = extract_first_link(message.get(
#                     "text", "") or message.get("html", ""))
#                 return {
#                     "subject": message.get("subject"),
#                     "link": link,
