"""
TEMPORARY EMAIL UTILITY FOR ETHERSCAN AUTOMATION
This utility module handles temporary email account creation and verification link extraction 
specifically for Etherscan API key generation. It provides a complete mail.tm integration that 
creates disposable email accounts, polls for incoming verification emails, and intelligently 
extracts Etherscan verification links from email content using multiple parsing strategies 
...
def extract_first_link(text):
    """Extract the first verification link from etherscan"""
    if not text:
        return None
    ...
    # Multiple patterns to catch etherscan verification links
    patterns = [
        r'https://[^"\s<>]*etherscan[^"\s<>]*verify[^"\s<>]*',
        r'https://[^"\s<>]*etherscan[^"\s<>]*confirm[^"\s<>]*',
        r'https://[^"\s<>]*etherscan[^"\s<>]*activation[^"\s<>]*',
        r'https://pro\.etherscan\.io/[^"\s<>]*verify[^"\s<>]*',
        r'https://pro\.etherscan\.io/[^"\s<>]*confirm[^"\s<>]*',
        r'https://[^"\s<>]*etherscan[^"\s<>]+',  # Fallback: any etherscan link
    ]
    ...
    try:
        ...
        for tag in soup.find_all("a", href=True):
            href = tag['href']
            if "etherscan" in href.lower() and any(keyword in href.lower() for keyword in ["verify", "confirm", "activation"]):
                print(f"🔗 Found verification link via BeautifulSoup: {href}")
                return href

        # Last resort: any etherscan link
        for tag in soup.find_all("a", href=True):
            href = tag['href']
            if "etherscan" in href.lower():
                print(f"🔗 Found etherscan link (fallback): {href}")
                return href
